  ...
  try {
    do {
      const url = CONFIG.apiUrl(
        `?method=user.getrecenttracks&user=${username}&limit=${limit}&page=${page}&from=${fromTimestamp}${toParam}&_t=${Date.now()}`,
      );
...
function aplicarCoresDinamicas(card, accentColor, format) {
  if (format === "story") {
    const textElements = card.querySelectorAll(".story-subtitle, .story-username, .footer-stat-label, .story-rank");
